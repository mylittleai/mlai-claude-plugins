...
description: >
  This skill should be used when the user invokes "/mtplan:save", asks to
  "save session state", "save progress", "prepare for session end",
  "write state before ending", or wants to update STATE.md before ending a work session.
argument-hint: ""
...
Before writing the save, verify PLAN.md is current:
...
This step prevents the most common save failure: STATE.md says "done" but PLAN.md checkboxes disagree.
...
The save is complete when a hypothetical fresh agent reading only CLAUDE.md, PLAN.md, and STATE.md could:
...
- Display a summary of the saved state.
...
- **Stale PLAN.md:** Writing a save without first updating PLAN.md checkboxes creates divergence. Always audit checkpoints first.
